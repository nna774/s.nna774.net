...
- **タイムラインに JS を基本的に書かない。** 素の form と 303 リダイレクトで完結
  させ、JS が無くても全機能が使える状態を保つ。ただし、それを壊さない範囲の
  小さな上乗せは許す。現状は `layout.html` 末尾の Cmd-Enter (Ctrl-Enter) 送信
  だけで、これは投稿ボタンの代替に過ぎない。
